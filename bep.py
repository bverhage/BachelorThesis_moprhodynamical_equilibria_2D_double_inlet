...
if True:
    def uvsolveFE(zetaStart,uStart,vStart,CStart,hStart,tStart,tEnd,Nt):
        ...
        if Nt==1:
            zetaEnd=zetaEnd-ht*(-LxN.dot((1-hEnd+zetaEnd)*uEnd)-LyN.dot((1-hEnd+zetaEnd)*vEnd))#+createt(Nx,Ny,boundarycondition,tStart)
            uEnd=uEnd-ht*(fhat*vEnd  -NU*LxD.dot(zetaEnd)-r*uEnd/(1-(hEnd-zetaEnd)))
            vEnd=vEnd-ht*(-fhat*uEnd -NU*LyD.dot(zetaEnd)-r*vEnd/(1-(hEnd-zetaEnd)))
            CEnd=CEnd#-ht*(-Lx.dot(CEnd*uEnd)-Ly.dot(CEnd*vEnd)-mu*A.dot(CEnd)+uEnd**2+vEnd**2-CEnd)
            hEnd=hEnd#-ht*(-uEnd**2-vEnd**2+CEnd)/1000
        else:
            ...
            #for i in tqdm(range(Nt)):
                ...
                if Boolean_forcing==True:
                    '''ontinutity equaiton'''
                    # f_zeta =-LxND.dot(DEPHT*uStart)+BoundaryForcing*fprime(tStart,i) # xdireciton
                    # f_zeta+=-LyD.dot(DEPHT*vStart)                                  # y direction
                    
                    # #zetaEnd  = f(tStart,i)*BoundaryForcing
                    # zetaEnd = (zetaStart-ht*f_zeta)#*(ONES-BoundaryForcing)
                    
                    # '''navier stockets u (x direction)'''
                    # f_u  = fhat*vStart                                                          # coriolis
                    # f_u += -NU*LxD.dot(zetaStart)- NU/(2*dx)*BoundaryForcing*f(tStart,i)   # surface 
                    ...
                    # #uEnd = BoundaryForcing*fprime(tStart,i)/(H-0+f(tStart,i))
                    # uEnd = EBoundary*0 
                    # uEnd += (uStart-ht*f_u)*(ONES-EBoundary)#-BoundaryForcing)
