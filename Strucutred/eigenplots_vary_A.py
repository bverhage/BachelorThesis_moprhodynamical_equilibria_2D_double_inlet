...
for i in range(np.size(LIST)):
    ...
    F_list[i]=TM.MaxNormOfU(TM.F(U,phi=np.pi*90/180,Ar=n_LIST[i]/100*P.A))
 ...
 #   eigen_vec2_maxh=np.max(np.real(h))
    ...
    #     eigvec_shift=power_iteration(A_shift,10000)
...
#ax3.plot(n_LIST/100,T_list_s,color='firebrick',marker="", linewidth=1.7, linestyle=':')
#ax3.plot(n_LIST/100,T_list_b,color='firebrick',marker="", linewidth=1.7, linestyle=':')
...
ax1.set_ylabel('wave amplitude ratio')
