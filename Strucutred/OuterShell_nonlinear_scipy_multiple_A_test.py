...
for i in range(np.size(LIST)):
    ...
    def func(U):
        return TM.F(U,phi=np.pi*90/180,Ar=n_LIST[i]/100*P.A)
    
    Ufinal=optimize.fsolve(func, U)
    
    j=0
    epsilon=10**(-9)
    Check=TM.MaxNormOfU(TM.F(Ufinal,phi=np.pi*90/180,Ar=n_LIST[i]/100*P.A))
    while Check>epsilon:
        
        print('\ fsolve loop \n j=%i \t ||F(U)||_max = %.2e < %.2e \n ' %(j,TM.MaxNormOfU(TM.F(Ufinal,phi=np.pi*90/180,Ar=n_LIST[i]/100*P.A)),epsilon))
        
        F_zetas,F_zetac,F_us,F_uc,F_vs,F_vc,F_C,F_h=TM.split_animation(TM.F(Ufinal,phi=np.pi*90/180,Ar=n_LIST[i]/100*P.A))
        ...
        NJ=TM.NumericalJacobian(Ufinal,phi=np.pi*90/180,Ar=n_LIST[i]/100*P.A)
        DeltaU=la.spsolve(NJ,TM.F(Ufinal,phi=np.pi*90/180,Ar=n_LIST[i]/100*P.A))
        ...
        F1=TM.MaxNormOfU(TM.F(Ufinal_hat10,phi=np.pi*90/180,Ar=n_LIST[i]/100*P.A));
        F2=TM.MaxNormOfU(TM.F(Ufinal_hat075,phi=np.pi*90/180,Ar=n_LIST[i]/100*P.A));
        F3=TM.MaxNormOfU(TM.F(Ufinal_hat05,phi=np.pi*90/180,Ar=n_LIST[i]/100*P.A));
        F4=TM.MaxNormOfU(TM.F(Ufinal_hat025,phi=np.pi*90/180,Ar=n_LIST[i]/100*P.A));
        F21=TM.MaxNormOfU(TM.F(Ufinal_hat01,phi=np.pi*90/180,Ar=n_LIST[i]/100*P.A))
        F22=TM.MaxNormOfU(TM.F(Ufinal_hat001,phi=np.pi*90/180,Ar=n_LIST[i]/100*P.A))
    ...
    print('\t fsolve loop stoped \n \t ||F(U)||_max = %.2e \n' %(TM.MaxNormOfU(TM.F(Ufinal,phi=np.pi*90/180,Ar=n_LIST[i]/100*P.A))))
    F_zetas,F_zetac,F_us,F_uc,F_vs,F_vc,F_C,F_h=TM.split_animation(TM.F(Ufinal,phi=np.pi*90/180,Ar=n_LIST[i]/100*P.A))
    print('\t | F(zetas) = %.2e \t | F(zetac) = %.2e \n \t | F(us) = %.2e \t | F(uc) = %.2e \n \t | F(vs) = %.2e \t | F(vc) = %.2e \n \t | F(C) = %.2e \t | F(h) = %.2e \n ' %(np.linalg.norm(F_zetas),np.linalg.norm(F_zetac),np.linalg.norm(F_us),np.linalg.norm(F_uc),np.linalg.norm(F_vs),np.linalg.norm(F_vc),np.linalg.norm(F_C),np.linalg.norm(F_h)))
    ...
    h_list[i]=np.max(h)
    F_list[i]=TM.MaxNormOfU(TM.F(Ufinal,phi=np.pi*90/180,Ar=n_LIST[i]/100*P.A))
