...
if BOOL_Total_Model:
    ...
    for n in n_LIST:
        if P.Nx==61:
            Load_LIST.append(str('Uphi%i_Nx60_test.npy'%(n)))
            Save_LIST.append(str('Uphi%i_Nx60_test_Ny5.npy'%(n)))
        else:
            print('\n \t \t WRONG Nx \n ')  
...
for i in range(np.size(Load_LIST)):
    print('\n ====================================================== \n \n \t -- currently at %i grad-- \n \n \t loading file ' %(n_LIST[i]),Load_LIST[i])
   
    U = np.load(Load_LIST[i])
    ...
    while Check>epsilon:
        ...
        print('\ fsolve loop \n j=%i \t ||F(U)||_max = %.2e < %.2e \n ' %(j,TM.MaxNormOfU(TM.F(Ufinal,phi=np.pi*n_LIST[i]/180)),epsilon))
        
        F_zetas,F_zetac,F_us,F_uc,F_vs,F_vc,F_C,F_h=TM.split_animation(TM.F(Ufinal,phi=np.pi*n_LIST[i]/180))
        ...
        NJ=TM.NumericalJacobian(Ufinal,phi=np.pi*n_LIST[i]/180)
        DeltaU=la.spsolve(NJ,TM.F(Ufinal,phi=np.pi*n_LIST[i]/180))
        ...
        F1=TM.MaxNormOfU(TM.F(Ufinal_hat10,phi=np.pi*n_LIST[i]/180));
        F2=TM.MaxNormOfU(TM.F(Ufinal_hat075,phi=np.pi*n_LIST[i]/180));
        F3=TM.MaxNormOfU(TM.F(Ufinal_hat05,phi=np.pi*n_LIST[i]/180));
        F4=TM.MaxNormOfU(TM.F(Ufinal_hat025,phi=np.pi*n_LIST[i]/180));
        F21=TM.MaxNormOfU(TM.F(Ufinal_hat01,phi=np.pi*n_LIST[i]/180))
        F22=TM.MaxNormOfU(TM.F(Ufinal_hat001,phi=np.pi*n_LIST[i]/180))
    ...
    print('\t fsolve loop stoped \n \t ||F(U)||_max = %.2e \n' %(TM.MaxNormOfU(TM.F(Ufinal,phi=np.pi*n_LIST[i]/180))))
    F_zetas,F_zetac,F_us,F_uc,F_vs,F_vc,F_C,F_h=TM.split_animation(TM.F(Ufinal,phi=np.pi*n_LIST[i]/180))
    print('\t | F(zetas) = %.2e \t | F(zetac) = %.2e \n \t | F(us) = %.2e \t | F(uc) = %.2e \n \t | F(vs) = %.2e \t | F(vc) = %.2e \n \t | F(C) = %.2e \t | F(h) = %.2e \n ' %(np.linalg.norm(F_zetas),np.linalg.norm(F_zetac),np.linalg.norm(F_us),np.linalg.norm(F_uc),np.linalg.norm(F_vs),np.linalg.norm(F_vc),np.linalg.norm(F_C),np.linalg.norm(F_h)))
    ...
    lambda_list[i]=np.nanmax(P.dx*P.dy*np.transpose(eigvec)*(NJ*eigvec)/(np.transpose(eigvec)*eigvec))
    ...
    if SAVE_BOOL==True:
        np.save(Save_LIST[i],Ufinal)
        print('\t saved ',Save_LIST[i] )
