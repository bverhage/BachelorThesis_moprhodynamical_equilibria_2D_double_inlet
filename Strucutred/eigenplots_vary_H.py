...
n_LIST= np.flip(np.arange(47,200,1))#np.flip(np.arange(47,200,1))
path=r'DATA\Equilibria_variation_of_paramters\Varied-H'
LIST=[]
for n in n_LIST:
    if P.Nx==61:
        LIST.append(str(path+r'\UH%i_Nx60_test_1.npy'%(n)))
    else:
        print('\n \t \t WRONG Nx \n ')
...
F_list_zetas=np.zeros(n_LIST.shape)
F_list_zetac=np.zeros(n_LIST.shape)

F_list_us=np.zeros(n_LIST.shape)
F_list_uc=np.zeros(n_LIST.shape)

F_list_vs=np.zeros(n_LIST.shape)
F_list_vc=np.zeros(n_LIST.shape)

F_list_C=np.zeros(n_LIST.shape)
F_list_h=np.zeros(n_LIST.shape)
...
for i in range(np.size(LIST)):
    ...
    F_list[i]=TM.MaxNormOfU(TM.F(U,phi=np.pi*90/180,Ar=P.A,H2=P.H2*n_LIST[i]/100))
    
    F_zetas,F_zetac,F_us,F_uc,F_vs,F_vc,F_C,F_h=TM.split_animation(TM.F(U,phi=np.pi*90/180,Ar=P.A,H2=P.H2*n_LIST[i]/100))
    F_list_zetas[i]=np.max(F_zetas)
    F_list_zetac[i]=np.max(F_zetac)
    F_list_us[i]=np.max(F_us)
    F_list_uc[i]=np.max(F_uc)
    F_list_vs[i]=np.max(F_vs)
    F_list_vc[i]=np.max(F_vc)
    F_list_C[i]=np.max(F_C)
    F_list_h[i]=np.max(F_h)
...
# ax.plot(n_LIST/100,P.H1-h_list,color='black',marker="", linewidth=2.3, linestyle='-')
# plt.ylim([P.H1,0])
# plt.xlim([np.min(n_LIST)/100,np.max(n_LIST)/100])
...
ax.plot(np.flip(np.arange(47,200,1))/100,np.load(r'DATA\variables_for_plots\eigenvalue_list_vary_h.npy')/(P.Nx*2),color='slategray',marker=".", linewidth=0, linestyle='-')
ax.plot(np.flip(np.arange(47,200,1))/100,np.load(r'DATA\variables_for_plots\eigenvalue_list_vary_h_Ny3.npy')/(P.Nx*4),color='black',marker=".", linewidth=0, linestyle='-')
...
ax.plot(np.flip(np.arange(47,200,1))/100,np.load(r'DATA\variables_for_plots\F_list_vary_H_Ny1.npy'),color='slategray',marker=".", linewidth=0, linestyle='-')
ax.plot(np.flip(np.arange(47,200,1))/100,np.load(r'DATA\variables_for_plots\F_list_vary_H_Ny3.npy'),color='black',marker=".", linewidth=0, linestyle='-')
plt.xlim([np.min(np.flip(np.arange(47,200,1)))/100,np.max(np.flip(np.arange(47,200,1)))/100])
ax.set_xlabel('inlet depth ratio ')
...
ax2=ax.twinx()
ax2.plot(n_LIST/100,u_list,color='royalblue',marker="", linewidth=2.3, linestyle='--')
ax2.tick_params(axis='y', labelcolor='royalblue')
plt.ylim([0,np.max(u_list)])
plt.xlim([0,np.max(n_LIST)/100])
ax2.set_ylabel('maximal water flow [m/s]',color='royalblue')

ax3=ax.twinx()
ax3.tick_params(axis='y', labelcolor='firebrick')
ax3.spines['right'].set_position(('outward', 60))
plt.ylim([min(np.min(T_list),0),np.max(T_list)])
ax3.plot(n_LIST/100,T_list,color='firebrick',marker="", linewidth=2.3, linestyle=':')
...
fig, ax= plt.subplots()
#ax.plot(n_LIST/100,F_list,color='black',marker=".", linewidth=0, linestyle='-')
ax.plot(n_LIST/100,F_list_zetas,marker=".", linewidth=0, linestyle='-')
ax.plot(n_LIST/100,F_list_zetac,marker=".", linewidth=0, linestyle='-')
ax.plot(n_LIST/100,F_list_us,marker=".", linewidth=0, linestyle='-')
ax.plot(n_LIST/100,F_list_uc,marker=".", linewidth=0, linestyle='-')
ax.plot(n_LIST/100,F_list_vs,marker=".", linewidth=0, linestyle='-')
ax.plot(n_LIST/100,F_list_vc,marker=".", linewidth=0, linestyle='-')
#ax.plot(n_LIST/100,F_list_C,marker=".", linewidth=0, linestyle='-')
#ax.plot(n_LIST/100,F_list_h,marker=".", linewidth=0, linestyle='-')
ax.legend(['$ zeta^{sin} $','$ zeta^{cos} $','$ u^{sin} $','$ u^{cos} $','$ v^{sin} $','$ v^{cos} $','$ matcal{C} $','$ h $'])
ax.set_xlabel('inlet depth ratio ')
