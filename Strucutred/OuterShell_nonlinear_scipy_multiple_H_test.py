...
if True:
    ...
    n_LIST=np.flip(np.arange(46,163,1))
    LIST=[]
    for n in n_LIST:
        if P.Nx==61:
...
for i in range(np.size(LIST)):
    ...
    if i==0:
        U = np.load(LIST[i])
    elif LIST[i]=='UH47_Nx60_test.npy':
        U = np.load('UH47_Nx60_test.npy')
    ...
    def func(U):
        return TM.F(U,phi=np.pi*90/180,Ar=P.A,H2=P.H2*n_LIST[i]/100)
    Ufinal=optimize.fsolve(func, U)
    j=0
    epsilon=10**(-9)
    Check=TM.MaxNormOfU(TM.F(U,phi=np.pi*90/180,Ar=P.A,H2=P.H2*n_LIST[i]/100))
    while Check>epsilon:
        
        print('\ fsolve loop \n j=%i \t ||F(U)||_max = %.2e < %.2e \n ' %(j,TM.MaxNormOfU(TM.F(Ufinal,phi=np.pi*90/180,Ar=P.A,H2=P.H2*n_LIST[i]/100)),epsilon))
        
        F_zetas,F_zetac,F_us,F_uc,F_vs,F_vc,F_C,F_h=TM.split_animation(TM.F(Ufinal,phi=np.pi*90/180,Ar=P.A,H2=P.H2*n_LIST[i]/100))
        ...
        NJ=TM.NumericalJacobian(Ufinal,phi=np.pi*90/180,Ar=P.A,H2=P.H2*n_LIST[i]/100)
        DeltaU=la.spsolve(NJ,TM.F(Ufinal,phi=np.pi*90/180,Ar=P.A,H2=P.H2*n_LIST[i]/100))
        ...
        F1=TM.MaxNormOfU(TM.F(Ufinal_hat10,phi=np.pi*90/180,Ar=P.A,H2=P.H2*n_LIST[i]/100));
        F2=TM.MaxNormOfU(TM.F(Ufinal_hat075,phi=np.pi*90/180,Ar=P.A,H2=P.H2*n_LIST[i]/100));
        F3=TM.MaxNormOfU(TM.F(Ufinal_hat05,phi=np.pi*90/180,Ar=P.A,H2=P.H2*n_LIST[i]/100));
        F4=TM.MaxNormOfU(TM.F(Ufinal_hat025,phi=np.pi*90/180,Ar=P.A,H2=P.H2*n_LIST[i]/100));
        F21=TM.MaxNormOfU(TM.F(Ufinal_hat01,phi=np.pi*90/180,Ar=P.A,H2=P.H2*n_LIST[i]/100))
        F22=TM.MaxNormOfU(TM.F(Ufinal_hat001,phi=np.pi*90/180,Ar=P.A,H2=P.H2*n_LIST[i]/100))
        ...
        Check=TM.MaxNormOfU(TM.F(Ufinal,phi=np.pi*90/180,Ar=P.A,H2=P.H2*n_LIST[i]/100))
        
    zetas,zetac,us,uc,vs,vc,C,h=TM.split_animation(Ufinal)
    print('\t fsolve loop stoped \n \t ||F(U)||_max = %.2e \n' %(TM.MaxNormOfU(TM.F(Ufinal,phi=np.pi*90/180,Ar=P.A,H2=P.H2*n_LIST[i]/100))))
    F_zetas,F_zetac,F_us,F_uc,F_vs,F_vc,F_C,F_h=TM.split_animation(TM.F(Ufinal,phi=np.pi*90/180,Ar=P.A,H2=P.H2*n_LIST[i]/100))
    print('\t | F(zetas) = %.2e \t | F(zetac) = %.2e \n \t | F(us) = %.2e \t | F(uc) = %.2e \n \t | F(vs) = %.2e \t | F(vc) = %.2e \n \t | F(C) = %.2e \t | F(h) = %.2e \n ' %(np.linalg.norm(F_zetas),np.linalg.norm(F_zetac),np.linalg.norm(F_us),np.linalg.norm(F_uc),np.linalg.norm(F_vs),np.linalg.norm(F_vc),np.linalg.norm(F_C),np.linalg.norm(F_h)))
    ...
    h_list[i]=np.max(h)
    F_list[i]=TM.MaxNormOfU(TM.F(Ufinal,phi=np.pi*90/180,Ar=P.A,H2=P.H2*n_LIST[i]/100))
